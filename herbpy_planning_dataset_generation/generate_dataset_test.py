...
def table_box_below_above(task_id, robot, env, G):
    ...
    small_box_pose[0,3], small_box_pose[2,3] = 0.75, 1.0
    tall_box_pose_1[0,3], tall_box_pose_1[2,3] = 0.75, 1.0
    tall_box_pose_2[0,3], tall_box_pose_2[2,3] = 0.75, 1.0
    ...
    for i in range(0,9):
        ...
        small_box_pose[1,3] = ypos
        tall_box_pose_1[1,3] = small_box_pose[1,3] - 0.3
        tall_box_pose_2[1,3] = small_box_pose[1,3] + 0.3
        ...
        c1 = numpy.array([table_pose[0,3], table_pose[1,3], table_pose[2,3]])
        c2 = numpy.array([tall_box_pose_1[0,3], tall_box_pose_1[1,3], tall_box_pose_1[2,3]])
        c3 = numpy.array([tall_box_pose_2[0,3], tall_box_pose_2[1,3], tall_box_pose_2[2,3]])
        c4 = numpy.array([small_box_pose[0,3], small_box_pose[1,3], small_box_pose[2,3]])

        cond = numpy.concatenate((c1, c2, c3, c4))
        G1 = G.copy()
        # binary_vec = [1]
        binary_vec, G1 = helper_test.get_binary_vector(G, G1, robot, env)
        start = []
        goal = []
        ...
        while(len(start)<50 or len(goal)<50):
            ...
            random_node, status = helper_test.get_valid_node(task_id, G, robot, env, table_pose, small_box_pose)
...
def table_box_left_right(task_id, robot, env, G):
    ...
    table_pose[0,3], table_pose[1,3], table_pose[2,3] = 0.8, 0, 0
    small_box_pose[0,3], small_box_pose[2,3] = 0.8, 1.0
    tall_box_pose_1[0,3], tall_box_pose_1[2,3] = 0.8, 1.0
    tall_box_pose_2[0,3], tall_box_pose_2[2,3] = 0.8, 1.0
...
def bookcase_front(task_id, robot, env, G):
    ...
    for b_y in bookcase_yp:
      for b_x in bookcase_xp:
        ...
        # binary_vec = helper_test.get_binary_vector(G, robot, env)
        start = []
        goal = []
        ...
        while(len(start)<50 or len(goal)<50):
            ...
            random_node_1, status = helper_test.get_valid_node(task_id, G, robot, env, bookcase_pose, None)
            random_node_2, status = helper_test.get_valid_node(task_id, G, robot, env, bookcase_pose, None)
            
            if(not helper_test.is_trivial(G.node[random_node_1]['state'], G.node[random_node_2]['state'], env, robot)):
